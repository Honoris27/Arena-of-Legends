...
import { Landmark, Coins, Clock, ArrowRight, ShieldCheck, History } from 'lucide-react';
...
const Bank: React.FC<BankProps> = ({ player, onDeposit, onCancelDeposit, onClaimDeposit }) => {
    ...
    const estimatedInterest = Math.floor(netDeposit * 0.10); // 10% interest for 7 days
    const totalReturn = netDeposit + estimatedInterest;
        ...
        <div className="max-w-5xl mx-auto pb-20">
            <div className="text-center mb-10">
                <h2 className="text-3xl cinzel font-bold text-yellow-500 mb-2 flex items-center justify-center gap-3">
                    <Landmark size={32} /> İmparatorluk Bankası
                </h2>
                <p className="text-slate-400">Altınlarını güvende tut ve faiz kazan.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl">
                    <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                        <Coins className="text-yellow-500"/> Para Yatırma (7 Gün Vadeli)
                    </h3>
                    ...
                    <div className="bg-slate-900/50 p-4 rounded-lg mb-6 text-sm text-slate-300 space-y-2">
                        ...
                        <div className="flex justify-between">
                            <span>İşlem Ücreti (Komisyon):</span>
                            <span className="text-red-400">%2</span>
                        </div>
                        <div className="flex justify-between">
                            <span>Vade Getirisi:</span>
                    ...
                    <div className="space-y-4">
                        ...
                        {depositAmount > 0 && (
                            <div className="bg-indigo-900/20 border border-indigo-500/30 p-3 rounded text-xs space-y-1">
                                ...
                                <div className="flex justify-between">
                                    <span className="text-slate-400">Net (Komisyon Sonrası):</span>
                                    <span className="text-white">{netDeposit}</span>
                                </div>
                                <div className="flex justify-between font-bold border-t border-indigo-500/30 pt-1 mt-1">
                                    <span className="text-green-400">7 Gün Sonra Alınacak:</span>
                                    <span className="text-yellow-400">{totalReturn}</span>
                                </div>
                            </div>
                        )}
                        ...
                        >
                            <ShieldCheck size={18} /> İşlemi Onayla
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 shadow-xl flex flex-col">
                    <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                        <History className="text-blue-500"/> Aktif Hesaplar
                    </h3>

                    <div className="flex-1 overflow-y-auto space-y-3 max-h-[400px]">
                        {player.bankDeposits.length === 0 ? (
                            <div className="text-center text-slate-500 py-10">Henüz vadeli hesabın yok.</div>
                        ) : (
                                ...
                                return (
                                    <div key={deposit.id} className="bg-slate-900 border border-slate-700 rounded-lg p-4 relative overflow-hidden group">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <div className="text-xs text-slate-500">Yatırılan Tutar</div>
                                            ...
                                            <div className="text-right">
                                                ...
                                                <div className="font-bold text-green-400 text-lg">+{Math.floor(deposit.amount * deposit.interestRate)}</div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 bg-slate-950 p-2 rounded mb-3">
                                            ...
                                            <span className={`text-xs font-mono ${isReady ? 'text-green-500 font-bold' : 'text-slate-300'}`}>
                                                {isReady ? "VADE DOLDU" : formatTime(timeLeft)}
                                            </span>
                                        </div>
                                        ...
                                        {isReady ? (
                                            ...
                                            >
                                                Parayı Çek ({total} Altın) <ArrowRight size={16}/>
                                            </button>
                                        ) : (
                                            <button 
                                            ...
                                            >
                                                Vadeyi Boz (Sadece Anapara: {deposit.amount})
                                            </button>
                                        )}
                                    </div>
                                )
